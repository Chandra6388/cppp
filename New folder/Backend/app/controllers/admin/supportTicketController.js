...
class Tickets {
    ...
    async getAllEmployees(req, res) {
        try {
            ...
            return res.send({ status: true, message: "Support team members fetched successfully.", data: employees, });

        } catch (error) {
            console.error("Error fetching support team members:", error);
            return res.status(500).send({ status: false, message: "An error occurred while fetching support team members. Please try again later.", error: error.message, });
        }
    }
    ...
    async assigneToSupport(req, res) {
        ...
        try {
            ...
            const updateTicket = await SupportTicketDB.findByIdAndUpdate(
                _id,
                { priority, category, assignee, status: "Progress" },
                { new: true }
            );
