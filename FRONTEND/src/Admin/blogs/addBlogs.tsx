...
import { addBlogs } from "@/service/admin/blogService"
...
const AddBlogs = () => {
  ...
  const config1 = useMemo(() => ({
    ...
    placeholder: "",
    buttons:
      "bold,italic,underline,strikethrough,ul,ol,paragraph,fontsize,font,link,image,video,table,align,undo,redo,cut,copy,paste,selectall",
  ...
  const handleSubmitBlog = async () => {
    ...
    await addBlogs(req)
      .then((res) => {
        if (res.status) {
          sweetAlert("success!", "🎉 Your blog has been successfully published!", "success", 2000);
          setFromData({
            title: "",
            description: ""
          })
