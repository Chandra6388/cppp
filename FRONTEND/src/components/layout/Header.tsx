...
const Header: React.FC<HeaderProps> = ({ onMenuClick, hideAccount = false }) => {
  ...
  const getUserInitial = () => {
    if (UserDetails?.FirstName) {
      return UserDetails?.FirstName.charAt(0);
    }
  };
    ...
    <header className="bg-[#031123] border-b border-[#112F59] p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-white font-medium hidden sm:block">Welcome, {UserDetails?.FirstName + " " + UserDetails?.LastName}</h2>
        </div>
        <div className="flex items-center gap-5" >
          {UserDetails?.Role == "ADMIN" ? "" : <Button variant="ghost" size="icon" className="text-white relative" style={{ background: "rgb(76 94 116)", borderRadius: "50%" }}>
          ...
          {!hideAccount && (
            <div className="relative" ref={dropdownRef}>
              {UserDetails?.profile_img ? <div>
                <img src={UserDetails?.profile_img} alt="Profile img" className="cursor-pointer" onClick={toggleDropdown} style={{ borderRadius: "50%", height: "38px", width: "38px" }} />
              </div> : <button className="flex items-center gap-2 cursor-pointer" onClick={toggleDropdown}>
                ...
                <div className="absolute right-0 mt-2 w-56 rounded-md shadow-lg bg-[#031123] border border-[#112F59] z-50">
                  <div className="py-2">
                    <div className="px-4 py-3 border-b border-[#112F59]">
                      <p className="text-sm text-white font-medium">{UserDetails?.FirstName + " " + UserDetails?.LastName}</p>
