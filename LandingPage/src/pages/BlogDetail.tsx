...
import { getBlogById , getTopReatedBlog} from '@/service/apiService'

const BlogDetail = () => {
  const location = useLocation()
  const [Blog, setBlog] = useState([])
  const [topThreeReatedBlog, setTopThreeReatedBlog] = useState([])
  ...
  const topReatedBlog = async () => {
    const req = {}
    await getTopReatedBlog(req)
      .then((res) => {
        if (res.status) {
          setTopThreeReatedBlog(res.data)
        }
        else {
          setTopThreeReatedBlog([])
        }
      })
      .catch((error) => {
        console.log("Error in fetching blog data", error)
      ...
      <div className="pt-32 pb-20">
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="creative-card p-8 md:p-12 rounded-3xl mb-16">
              <div className="prose prose-lg prose-invert max-w-none">
                <p
                  className="text-gray-400 font-light leading-relaxed mb-6 group-hover:text-gray-300 transition-colors duration-300"
                  dangerouslySetInnerHTML={{ __html: Blog?.[0]?.discription }}
                ></p>
              </div>
            </div>
            {topThreeReatedBlog.length > 0 && (
              <div>
                <h3 className="text-3xl font-light text-white mb-8 text-center">Related Articles</h3>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {topThreeReatedBlog.map((relatedPost) => (
